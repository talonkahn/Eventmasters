...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (order.status === 'paid') {
      ...
      return json({ success: true, order: full });
    }

    // Re-verify with Flutterwave V4
    ...
    if (verifyData.status !== 'success' || verifyData.data?.status !== 'successful') {
      return json({ error: 'Payment not verified by Flutterwave', flwStatus: verifyData.data?.status }, 400);
    }
